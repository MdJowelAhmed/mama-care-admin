...
export default function ReportsPage() {
  ...
  const { data: reportDetails, isLoading: isLoadingDetails } =
    useGetSingleReportQuery(selectedReportId || "", {
  ...
  const meta = reportsData?.data?.meta ?? {
    total: 0,
    limit: 10,
    page: 1,
    totalPage: 1,
  };
  const reportDetail = (reportDetails?.data?.data || reportDetails?.data) as
    | Report
    | undefined;
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Report Topic",
      className: "",
      render: (value: ReportTopic) =>
        reportTopicLabels[value as keyof ReportTopicLabels] || value,
    },
    {
      key: "createdAt",
      header: "Date",
    ...
    },
    {
      ...
      render: (value: ReportStatus) => (
        <div className="flex items-center space-x-2">
          ...
          <Badge
            className={
              statusColors[value as keyof StatusColors] ||
      ...
      render: (_: any, report: Report) => (
        <div className="flex items-center space-x-2">
          <Select
            ...
            onValueChange={(value) =>
              handleStatusUpdate(report._id, value as ReportStatus)
            }
            ...
            <>
              ...
              <div className="space-y-6 py-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Badge
                      className={
                        statusColors[
                          reportDetail.status as keyof StatusColors
                        ] || "bg-gray-100 text-gray-800"
                      }
                    >
                      {reportDetail.status
                ...
                <div>
                  ...
                  <p>
                    {reportTopicLabels[
                      reportDetail.topic as keyof ReportTopicLabels
                    ] || reportDetail.topic}
                  </p>
                </div>
                    ...
                    <CardContent>
                      <div className="flex items-center space-x-3">
                        <Avatar className="h-10 w-10">
                          <AvatarImage
                            src={reportDetail.reporterId?.profileImage}
                            alt={reportDetail.reporterId?.name}
                          />
                          <AvatarFallback>
                            {reportDetail.reporterId?.name
                              ?.split(" ")
                              .map((n) => n[0])
                              .join("") || "U"}
                          </AvatarFallback>
                        ...
                        <div>
                          <p className="font-medium">
                            {reportDetail.reporterId?.name || "Unknown"}
                          </p>
                          <p className="text-sm text-gray-500">
                            {reportDetail.reporterId?.role || "N/A"}
                          </p>
                        </div>
                      </div>
                    </CardContent>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="flex items-center space-x-3">
                        <Avatar className="h-10 w-10">
                          <AvatarImage
                            src={reportDetail.reportedUserId?.profileImage}
                            alt={reportDetail.reportedUserId?.name}
                          />
                          <AvatarFallback>
                            {reportDetail.reportedUserId?.name?.charAt(0) ||
                              "U"}
                          </AvatarFallback>
                        </Avatar>
                        <div>
                          <p className="font-medium">
                            {reportDetail.reportedUserId?.name || "Unknown"}
                          </p>
                          <p className="text-sm text-gray-500">
                            {reportDetail.reportedUserId?.role || "N/A"}
                          </p>
                        </div>
                      </div>
                    </CardContent>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <Card>
                        <CardContent className="pt-4">
                          ...
                          <p>
                            ${reportDetail.bookingId.totalPayable?.toFixed(2)}
                          </p>
                        </CardContent>
                      </Card>

                      {reportDetail.bookingId.bookingType === "HOURLY" &&
                        reportDetail.bookingId.hourlyBooking && (
                          <Card className="md:col-span-2">
                            <CardContent className="pt-4">
                              ...
                              <p>
                                {new Date(
                                  reportDetail.bookingId.hourlyBooking.date
                                ).toLocaleDateString()}{" "}
                                |
                                {reportDetail.bookingId.hourlyBooking.startTime}{" "}
                                - {reportDetail.bookingId.hourlyBooking.endTime}
                              </p>
                            </CardContent>
                          </Card>
                        )}

                      {reportDetail.bookingId.bookingType === "FULL_DAY" &&
                        reportDetail.bookingId.fullDayBooking &&
                        reportDetail.bookingId.fullDayBooking.fullDays.length >
                          0 && (
                          <Card className="md:col-span-2">
                            <CardContent className="pt-4">
                              <div className="flex items-center space-x-2 mb-2">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {reportDetail.bookingId.fullDayBooking.fullDays.map(
                                  (day, index) => (
                ...
                <div className="grid grid-cols-2 gap-4 text-sm text-gray-500">
                  ...
                  <div>
                    <p>Updated At:</p>
                    <p className="font-medium">
                      {new Date(reportDetail.updatedAt).toLocaleString()}
                    </p>
                  </div>
                </div>
              </div>
